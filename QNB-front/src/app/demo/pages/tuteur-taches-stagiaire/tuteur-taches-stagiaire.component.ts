...
export class TuteurTachesStagiaireComponent implements OnInit {
  ...
  loadTaches() {
    this.http.get<any[]>(`http://192.168.5.128:31615/api/taches/stagiaire/${this.stagiaireId}`).subscribe({
  ...
  validerRendu(tache: any = this.selectedTache) {
    ...
    this.http.put(`http://192.168.5.128:31615/api/taches/${tache.id}/rendu/valider`, {}, { params: { commentaire } }).subscribe({
      next: () => {
  ...
  rejeterRendu(tache: any = this.selectedTache) {
    const commentaire = tache.commentaireTuteur || '';
    this.http.put(`http://192.168.5.128:31615/api/taches/${tache.id}/rendu/rejeter`, {}, { params: { commentaire } }).subscribe({
      next: () => {
