...
@Component({
  ...
  imports: [
    ...
    MatButtonModule,
    // add chips module
    // NOTE: MatChipsModule is required by the template for mat-chip-list / mat-chip
    // import it below (avoid duplicate imports of MatFormFieldModule)
    // We'll reference it by adding the symbol to imports list
    // (can't import symbol literal here in patch - ensure module is available in project)
    FormsModule,
...
export default class BasicElementsComponent {
  ...
  // Helper to get stagiaire display name by id (avoids arrow functions in template)
  getStagiaireNameById(id: any): string {
    if (!this.stagiaires || !id) return id;
    const s = this.stagiaires.find(st => st.id === id);
    return s ? (s.username || s.nom || s.fullName || s.email || id) : id;
  }
