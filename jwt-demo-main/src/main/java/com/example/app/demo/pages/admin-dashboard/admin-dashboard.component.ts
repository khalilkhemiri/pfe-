...
export class AdminDashboardComponent implements OnInit {
  ...
  loadDashboardData(): void {
    ...
    this.authService.getPendingUsers().pipe(catchError(() => of([]))).subscribe({
      next: (pending) => {
        this.pendingUsers = (pending || []).map(u => ({ ...u, _selectedRole: u._selectedRole || 'STAGIAIRE', _selectedTuteur: null }));
      }
    });
    ...
    if (this.confirmType === 'approve') {
      this.authService.assignRole(user.id, role).pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => {
          ...
          // If assigning STAGIAIRE, also assign a tuteur if selected
          if ((role === 'STAGIAIRE' || role === 'stagiaire') && user._selectedTuteur) {
            this.authService.assignTuteur(user.id, user._selectedTuteur).pipe(takeUntil(this.destroy$)).subscribe({
              next: () => {
                this.loadDashboardData();
                this.cancelConfirm();
              },
              error: (err) => {
                console.error('Erreur assignation tuteur:', err);
                this.cancelConfirm();
              }
            });
          } else {
  ...
  selectPendingUser(user: any) {
    this.selectedPendingUser = user;
    // Ensure the selected user has role/tutor defaults
    if (!this.selectedPendingUser._selectedRole) this.selectedPendingUser._selectedRole = 'STAGIAIRE';
    if (this.selectedPendingUser._selectedTuteur === undefined) this.selectedPendingUser._selectedTuteur = null;
  }
